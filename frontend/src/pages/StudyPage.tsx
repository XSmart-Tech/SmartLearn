...
import type { CardsSliceState, Status } from '@/store/cardsSlice'
import { fetchLibraries, fetchLibraryById } from '@/store/librariesSlice'
...
export default function StudyPageMerged() {
  ...
  const filtered = useMemo(() => {
    ...
    const base = q
      ? cards.filter(c => `${c.front} ${c.back} ${c.description ?? ''}`.toLowerCase().includes(q))
      : cards
    ...
    return copy
  }, [cards, query, shuffleOn, shuffleSeed])
          ...
          {started ? (
            ...
            <div className="rounded-2xl border p-6 space-y-4 bg-card">
              ...
              <div className="text-left">
                ...
                <div className="space-y-3">
                  {quizCards.length === 0
                ? <div className="text-sm text-muted-foreground">Không có câu hỏi.</div>
                : quizCards.map((c, idx) => (
