      ...
      <PopoverTrigger asChild>
        <div ref={triggerRef} className={`relative ${className}`}>
          ...
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 pointer-events-none">
            ...
            >
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : t('common.find')}
            </Button>
          </div>
        </div>
      </PopoverTrigger>
      ...
      >
        {items.length === 0 ? (
          ...
          <ul
            ...
            style={{ maxHeight }}
            role="listbox"
            aria-label={t('common.userSearchResults')}
          >
            {items.map((u, idx) => {
                  ...
                  >
                    ...
                    <div className="flex min-w-0 flex-1 items-center justify-between">
                      <div className="min-w-0">
                        ...
                        <Small className="truncate text-muted-foreground">{u.email}</Small>
                      </div>
                      <span className="ml-3 shrink-0 rounded-full border px-2 py-0.5 text-[11px] text-muted-foreground">
                        {t('common.user')}
                      </span>
                    </div>
                  </button>
