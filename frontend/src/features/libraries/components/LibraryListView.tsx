...
interface LibraryListViewProps {
  ...
  viewMode?: 'list' | 'grid'
  showActions?: boolean
  filterByRole?: 'owner' | 'contributor' | 'viewer' | 'all'
...
export default function LibraryListView({
  ...
  const finalSortOptions = customSortOptions || defaultSortOptions
  ...
  // Lọc thư viện theo role nếu cần
  const filteredLibs = React.useMemo(() => {
    if (filterByRole === 'all' || !uid) return libs

    return libs.filter(lib => {
      if (lib.ownerId === uid) return filterByRole === 'owner'
      if (lib.shareRoles && lib.shareRoles[uid]) return lib.shareRoles[uid] === filterByRole
      return filterByRole === 'viewer'
    })
  }, [libs, filterByRole, uid])
